...
$nomad_blog_date = get_theme_mod('nomad_blog_date', true);
$nomad_blog_comments = get_theme_mod('nomad_blog_comments', true);
$nomad_blog_author = get_theme_mod('nomad_blog_author', true);
$nomad_blog_cat = get_theme_mod('nomad_blog_cat', true);
            ...
            <div class="blogBlock__meta mb-20">
                <ul class="blogBlock__meta__list">
                    <?php if (!empty($nomad_blog_date)): ?>
                        <li><span class="blogBlock__meta__text"><i
                                        class="far fa-calendar-check"></i> <?php the_time(get_option('date_format')); ?></span>
                        </li>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($nomad_blog_author)): ?>
                        <li><a class="blogBlock__meta__text"
                               href="<?php print esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><i
                                        class="far fa-user"></i> <?php print get_the_author(); ?> </a></li>
                    <?php endif; ?>

                    <?php if (!empty($nomad_blog_comments)): ?>
                        <li><a class="blogBlock__meta__text" href="<?php comments_link(); ?>"><i
                                        class="fal fa-comments"></i> <?php comments_number(); ?></a></li>
                    <?php endif; ?>
                </ul>
            </div>
...
<?php
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('col-md-6 col-lg-4 mt-5 wow fadeInUp d-flex align-items-stretch'); ?>
             data-wow-delay=".3s">
        <div class="" data-wow-delay="1.2s"
             style="visibility: visible; animation-delay: 1.2s; animation-name: fadeInUp;">
            <div class="blog-grid">
                <?php if (has_post_thumbnail()): ?>
                    <div class="blog-grid-img position-relative"><?php the_post_thumbnail('full', ['class' => '']); ?></div>
                <?php endif; ?>
                <div class="blog-grid-text p-4">
                    <h3 class="h5 mb-3"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h3>
                    <p class="display-30 "> <?php the_excerpt(); ?></p>
                    <?php
                    ...
                    <div class="position-relative">
                        <div class="meta meta-style2 " >
                            <ul>
                                <?php if (!empty($nomad_blog_date)): ?>
                                    <li>
                                        <span><i class="fas fa-calendar-alt"></i> <?php the_time(get_option('date_format')); ?></span>
                                    </li>
                                <?php endif; ?>
                                <?php if (!empty($nomad_blog_author)): ?>
                                    <li><a class="blogBlock__meta__text"
                                           href="<?php print esc_url(get_author_posts_url(get_the_author_meta('ID'))); ?>"><i
                                                    class="fas fa-user"></i> <?php print get_the_author(); ?> </a></li>
                                <?php endif; ?>
                                <?php if (!empty($nomad_blog_comments)): ?>
